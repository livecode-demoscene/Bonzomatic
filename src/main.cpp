...
int main( int argc, const char * argv[] )
{
  ...
  char szShader[ 65535 ];
  char szError[ 4096 ];
  FILE * f = fopen( Renderer::szDefaultShaderFilename, "rb" );
  if ( f )
  {
  ...
  if ( !shaderInitSuccessful )
  {
    ...
    std::string sDefShader = Renderer::szDefaultShader;
...
#define FKEY(x) ((x)+281)
    ...
    if ( newShader )
    {
      ...
      FILE * f = fopen( Renderer::szDefaultShaderFilename, "wb" );
      if ( f )
      {
  ...
  {
    Misc::ExecuteCommand( sPostExitCmd.c_str(), Renderer::szDefaultShaderFilename );
  }
