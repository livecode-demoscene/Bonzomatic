...
const char * szDefaultShaderFilename = "shader.dx11.hlsl";
const char szDefaultShader[ 65536 ] =
    ...
    case WM_LBUTTONDOWN:
      {
        ...
        mouseEventBuffer[ mouseEventBufferCount ].x = (float)GET_X_LPARAM( lParam );
        mouseEventBuffer[ mouseEventBufferCount ].y = (float)GET_Y_LPARAM( lParam );
...
bool InitWindow( Renderer::Settings * pSetup )
{
  WNDCLASS WC;
  ...
  if ( pSetup->windowMode == WINDOWMODE_WINDOWED ) wStyle |= WS_OVERLAPPED | WS_CAPTION;
...
bool bVsync = false;
bool InitDirect3D( Renderer::Settings * pSetup )
{
  ...
  desc.Windowed = pSetup->windowMode != WINDOWMODE_FULLSCREEN;
...
bool InitD3D11QuadRendering( Renderer::Settings * settings )
{
  ID3DBlob * pCode = NULL;
...
bool InitD3D11GUIRendering( Renderer::Settings * settings )
{
  ID3DBlob * pCode = NULL;
  ID3DBlob * pErrors = NULL;
